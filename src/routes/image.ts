...
//         console.log('Eval image :', evaluatedImage);
...
imageRouter.post('/', async (req: Request, res: Response) => {
    ...
    try {
        ...
        const evaluationResponse = await gptService.evaluateImage(imageUrl);
        const parsedEvaluation = JSON.parse(evaluationResponse);
        console.log('OpenAI API response:', parsedEvaluation);
        

        if (!parsedEvaluation || typeof parsedEvaluation !== 'object') {
        ...
        res.status(201).json({ message: 'Image evaluated successfully', evaluation: parsedEvaluation.evaluation });
    } catch (error: any) {
...
imageRouter.delete('/', async (req: Request, res: Response) => {
    ...
    try {
        ...
        console.error('Error removing image and evaluation:', error);
        res.status(500).json({ error: error.message });
    }
});
