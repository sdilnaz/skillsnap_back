...
class GptService {
  ...
  private generateSystemPromptWithLessons(lessons: string[]): string {
    ...
    return `
           ...
           return JSON format 
            indicate bold text with '###' (use only 3 '#' signs, do not use '**'
  ...
  async generateLesson(lessonTitle: string, userId: string) {
    try {
      ...
      const systemPromptWithLessons =
        this.generateSystemPromptWithLessons(existingLessons);
      ...
      if (error.message === "Invalid topic") {
        throw new Error(
          "Invalid topic. Please enter a valid photography-related topic."
        );
      }
